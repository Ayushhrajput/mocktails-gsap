    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setCurrentIndex((currentIndex) => {
                return currentIndex + 1 >= mocktails.length? 0: currentIndex +1
            })
        }, 4000);
        ...
        <section id={prop} aria-labelledby='menu-heading' className='relative bg-gray-200 py-4'>
            ...
            <nav id='cocktails-tab' aria-label='cocktail navigation' className=' flex justify-around gap-4 bg-white px-4 shadow-sm p-2 m-4 w-max md:w-xl mx-auto rounded-4xl'>
                {mocktails.map((mocktail, index) => {
                    ...
                    return (
                        <button key={mocktail.id} className={`${isActive? '': 'text-gray-600'} text-sm md:text-lg `} onClick={() => (setCurrentIndex(index))}>{mocktail.name}</button>
                    )
                })}
            </nav>
            <div className='content'>
                ...
                <div className='w-full md:w-80 h-160 md:h-120 mx-auto'>
                        {
                            <img key={currentIndex} src={sliderlists[currentIndex].image} alt="mocktail" className='w-full h-full object-cover' />
                        }
                ...
                <div className=' absolute top-1/6 md:top-1/2 md:left-1/8 px-4'>
                        <div className=' text-4xl text-white  md:text-black font-bold'>{mocktails[currentIndex].name}</div>
